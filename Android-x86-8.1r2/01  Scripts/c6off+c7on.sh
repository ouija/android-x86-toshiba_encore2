...
#title:       c6off+c7on.sh
#description: Disables all C6 and enables all C7 core states for Cherrytrail CPUs
#author:      Wolfgang Reimer <linuxball (at) gmail.com> (updated for Cherrytrail by ouija)
#date:        2016014
#version:     1.0    
...
# Iterate through each core state and for Cherrytrail (CHT) disable all C6 
...
for cpu in cpu[0-9]*; do
	for dir in $cpu/cpuidle/state*; do
		...
		case $name in
			#C6*-CHT) disable 1;;
			#C7*-CHT) disable 0;;
			# values do not contain CHT in them, leaving for reference
			C6*) disable 1;;
			C7*) disable 0;;
		esac
		cd ../../..
